...
extern "C"
{
	...
	uint32_t StackRAM[THREAD_NUM * THREAD_STACK_SIZE];
	
	volatile uint32_t UserThrownHardFault = 0;
}
...
void Kernel::Launch(void)
{
	...
	//Let's go
	CurrentStackPtr = &Kernel::GetCurrentThread()->Stack;
	KernelJumpStart();
}
...
extern "C"
{
	void RunScheduler(uint8_t state)
	{
		...
		if (CurrentStackPtr == 0)
		{
			...
			Kernel::ConfigureMPURegion(Kernel::CurrentThread, (uint32_t)&StackRAM[_stackStart], MPU_REGION_SIZE_32B, false);
			...
			Kernel::ConfigureMPURegion(Kernel::CurrentThread, (uint32_t)&StackRAM[_stackStart], MPU_REGION_SIZE_32B, true);
...
void Kernel::SetSystemClock(KernelClockSpeed clock)
{
	...
	if(clock == KernelClockSpeed::CLOCK_216MHZ)
	{
...
void Kernel::ConfigureMPURegion(uint8_t number, uint32_t address, uint8_t size, bool allow)
{
	...
	if (allow)
		...
		MPUStruct.AccessPermission = MPU_REGION_PRIV_RO_URO;    //MPU_REGION_FULL_ACCESS; //MPU_REGION_PRIV_RO_URO
